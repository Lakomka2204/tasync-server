...
export class FolderController {
    ...
    @Get()
    @HttpCode(200)
    async getAccountFolders(@Req() req: Request): Promise<FolderInfoDto[]> {
        const account: Account = JSON.parse(req.headers.authorization);
        return plainToInstance(
            FolderInfoDto,
            await this.folderService.getFolders(account),
        );
    }
    @Get(':name')
    @HttpCode(200)
    async getFolderInfo(
        ...
        const account: Account = JSON.parse(req.headers.authorization);
        const folder = await this.folderService.getFolderByName({
            ownerId: account.id,
            folderName,
        });
    ...
    async createFolder(
        ...
        if (!folder)
            throw new BadRequestException(
                `Folder with specified name already exists`,
            );
    }
    ...
    async deleteFolder(
        ...
        const account: Account = JSON.parse(req.headers.authorization);
        const isDeleted = await this.folderService.deleteFolder({
            ownerId: account.id,
            folderName,
        });
        if (!isDeleted) throw new NotFoundException('Folder is not found');
    }
    ...
    async createSnapshot(
        ...
        let folder = await this.folderService.getFolderByName({
            folderName,
            ownerId: account.id,
        });
        if (!folder)
            folder = await this.folderService.createFolder({
                ownerId: account.id,
                folderName,
            });
        // check commit
        ...
        const uniqueName = this.folderService.composeUniqueId({
            ownerId: account.id,
            folderName: folder.id.toString(),
            commit: newCommit,
        });
        // add archiving into queue
        await this.archiveService.addToQueue(
            uniqueName,
            files.filter((x) => x.size > 0),
        );
        // create commit in db
        await this.folderService.createCommit({
            ownerId: account.id,
            folderName,
            commit: newCommit,
        });
            ...
            switch (state) {
                case 'active':
                    throw new ServiceUnavailableException(
                        'Your files are being processed right now',
                    );
                case 'waiting':
                    throw new ServiceUnavailableException(
                        'Your files are in queue right now',
                    );
                default:
                    throw new UnprocessableEntityException(
                        'Your files are unaccessible right now',
                    );
            }
        }
